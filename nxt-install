...
#
# This script installs an NXT node (https://www.jelurida.com/nxt) on a debian server.
...
ARCHIVALDEFAULT=no
MAINNET_ADMIN_PASSWORD=tiachop1
...
NXT_MAINNET_FOLDER="nxt-mainnet"
NXT_TESTNET_FOLDER="nxt-testnet"

NXT_MAINNET_SERVICE="nxt-mainnet"
NXT_TESTNET_SERVICE="nxt-testnet"
...
NXT_MAINNET_SERVICE_FILE_CONTENT="
[Unit]
Description=NXT-Mainnet
After=syslog.target
After=network.target
[Service]
RestartSec=2s
Type=simple
WorkingDirectory=/home/${LOCAL_USER}/${NXT_MAINNET_FOLDER}/
ExecStart=/bin/bash /home/${LOCAL_USER}/${NXT_MAINNET_FOLDER}/run.sh
Restart=always
[Install]
...
NXT_TESTNET_SERVICE_FILE_CONTENT="
[Unit]
Description=NXT-Testnet
After=syslog.target
After=network.target
[Service]
RestartSec=2s
Type=simple
WorkingDirectory=/home/${LOCAL_USER}/${NXT_TESTNET_FOLDER}/
ExecStart=/bin/bash /home/${LOCAL_USER}/${NXT_TESTNET_FOLDER}/run.sh
Restart=always
[Install]
WantedBy=multi-user.target
"
...
###################################################################################################
echo ""
date +"%Y-%m-%d %H:%M:%S || [INFO] NXT install script started"
# Verification Checks
if [ $UID -eq 0 ]; then
...
UPDATE_NXT_NODES_SCRIPT_CONTENT="
#!/bin/bash
###################################################################################################
# CONFIGURATION
###################################################################################################
...
###
date +\"%Y-%m-%d %H:%M:%S || [INFO] Checking for new NXT release\"
...
if [ -z \${REMOTE_VERSION} ] || [ -z \${LOCAL_VERSION} ]; then
  echo \"[WARNING] Failed to check NXT versions\"
  exit 1
fi
if [ \${REMOTE_VERSION} != \${LOCAL_VERSION} ]; then
  echo \"[INFO] downloading new NXT release ...\"
  wget https://www.jelurida.com/nxt-client.zip -q --show-progress
  wget https://www.jelurida.com/nxt-client.zip.asc -q --show-progress
  gpg --with-fingerprint --verify nxt-client.zip.asc nxt-client.zip
  echo \"\" && echo \"[INFO] unzipping new NXT release ...\"
  unzip -qq nxt-client.zip
  if [ \${UPDATE_MAINNET_NODE} == true ]; then
      echo \"\" && echo \"[INFO] stopping NXT mainnet service ...\"
      sudo systemctl stop ${NXT_MAINNET_SERVICE}.service
      echo \"\" && echo \"[INFO] installing new NXT release ...\"
      mkdir nxt-mainnet-update
      mv ./${NXT_MAINNET_FOLDER}/conf/nxt.properties ./nxt-mainnet-update/nxt.properties
      sudo mv ./${NXT_MAINNET_FOLDER}/nxt_db/ ./nxt-mainnet-update/nxt_db/
      rm -rf ./${NXT_MAINNET_FOLDER}/
      cp -r ./nxt ./${NXT_MAINNET_FOLDER}
      mv ./nxt-mainnet-update/nxt.properties ./${NXT_MAINNET_FOLDER}/conf/nxt.properties
      sudo mv ./nxt-mainnet-update/nxt_db/ ./${NXT_MAINNET_FOLDER}/nxt_db/
      echo \"\" && echo \"[INFO] restarting NXT mainnet service ...\"
      sudo systemctl start ${NXT_MAINNET_SERVICE}.service
  fi
  if [ \${UPDATE_TESTNET_NODE} == true ]; then
      echo \"\" && echo \"[INFO] stopping NXT testnet service ...\"
      sudo systemctl stop ${NXT_TESTNET_SERVICE}.service
      echo \"\" && echo \"[INFO] installing new NXT release ...\"
      mkdir nxt-testnet-update
      mv ./${NXT_TESTNET_FOLDER}/conf/nxt.properties ./nxt-testnet-update/nxt.properties
      sudo mv ./${NXT_TESTNET_FOLDER}/nxt_test_db/ ./nxt-testnet-update/nxt_test_db/
      rm -rf ./${NXT_TESTNET_FOLDER}/
      cp -r ./nxt ./${NXT_TESTNET_FOLDER}
      mv ./nxt-testnet-update/nxt.properties ./${NXT_TESTNET_FOLDER}/conf/nxt.properties
      sudo mv ./nxt-testnet-update/nxt_test_db/ ./${NXT_TESTNET_FOLDER}/nxt_test_db/
      echo \"\" && echo \"[INFO] restarting NXT testnet service ...\"
      sudo systemctl start ${NXT_TESTNET_SERVICE}.service
  fi
  echo \"\" && echo \"[INFO] cleaning up ...\"
  rm -rf ./nxt-mainnet-update ./nxt-testnet-update
  rm -rf nxt nxt-client.zip nxt-client.zip.asc
  echo \"\" && echo \"[INFO] done. NXT nodes updated\"
else
  echo \"\" && echo \"[INFO] No update available\"
fi
"

OPTIMIZE_NXT_NODES_SCRIPT_CONTENT="
#!/bin/bash
###################################################################################################
# CONFIGURATION
###################################################################################################
...
# MAIN
###################################################################################################
date +\"%Y-%m-%d %H:%M:%S || [INFO] Starting NXT Optimization\"
  if [ \${OPTIMIZE_MAINNET_NODE} == true ]; then
      echo \"\" && echo \"[INFO] stopping NXT mainnet service ...\"
      sudo systemctl stop ${NXT_MAINNET_SERVICE}.service
      echo \"\" && echo \"[INFO] Optimizing mainnet node ...\"
      cd /home/${LOCAL_USER}/${NXT_MAINNET_FOLDER}
      /bin/bash compact.sh
      echo \"\" && echo \"[INFO] restarting NXT mainnet service ...\"
      sudo systemctl start ${NXT_MAINNET_SERVICE}.service
  fi
  if [ \${OPTIMIZE_TESTNET_NODE} == true ]; then
      ...
      cd /home/${LOCAL_USER}/${NXT_TESTNET_FOLDER}
      /bin/bash compact.sh
      echo \"\" && echo \"[INFO] restarting NXT testnet service ...\"
      sudo systemctl start ${NXT_TESTNET_SERVICE}.service
  fi
  echo \"\" && echo \"[INFO] done. NXT nodes optimized\"
"
...
echo "" && echo "[INFO] downloading NXT ..."
cd ~
wget https://www.jelurida.com/nxt-client.zip -q --show-progress
wget https://www.jelurida.com/nxt-client.zip.asc -q --show-progress
...
echo "" && echo "[INFO] unzipping NXT ..."
unzip -qq nxt-client.zip


if [ ${INSTALL_MAINNET_NODE} == true ]; then

    echo "" && echo "[INFO] creating NXT mainnet folder ..."
    cp -r nxt ${NXT_MAINNET_FOLDER}


    echo "" && echo "[INFO] creating NXT mainnet configuration ..."
    echo "${NXT_MAINNET_PROPERTIES_FILE_CONTENT}" > ${NXT_MAINNET_FOLDER}/conf/nxt.properties


    echo "" && echo "[INFO] creating NXT mainnet service ..."
    sudo mkdir -p /etc/systemd/system
    echo "${NXT_MAINNET_SERVICE_FILE_CONTENT}" | sudo tee /etc/systemd/system/${NXT_MAINNET_SERVICE}.service > /dev/null


    echo "" && echo "[INFO] enabling NXT mainnet service ..."
    sudo systemctl enable ${NXT_MAINNET_SERVICE}.service


    if [ ${DOWNLOAD_MAINNET_BLOCKCHAIN} == true ]; then

        echo "" && echo "[INFO] downloading mainnet blockchain ..."
        wget https://www.jelurida.com/NXT-nxt_db.zip -q --show-progress

        echo "" && echo "[INFO] unzipping mainnet blockchain ..."
        unzip -qq NXT-nxt_db.zip

        echo "" && echo "[INFO] moving mainnet blockchain to NXT mainnet folder ..."
        mv nxt_db/ nxt-mainnet/
    fi
fi
...
if [ ${INSTALL_TESTNET_NODE} == true ]; then
    ...
    echo "" && echo "[INFO] creating NXT testnet folder ..."
    cp -r nxt ${NXT_TESTNET_FOLDER}


    echo "" && echo "[INFO] creating NXT testnet configuration ..."
    echo "${NXT_TESTNET_PROPERTIES_FILE_CONTENT}" > ${NXT_TESTNET_FOLDER}/conf/nxt.properties


    echo "" && echo "[INFO] creating NXT testnet service ..."
    sudo mkdir -p /etc/systemd/system
    echo "${NXT_TESTNET_SERVICE_FILE_CONTENT}" | sudo tee /etc/systemd/system/${NXT_TESTNET_SERVICE}.service > /dev/null


    echo "" && echo "[INFO] enabling NXT testnet service ..."
    sudo systemctl enable ${NXT_TESTNET_SERVICE}.service


    if [ ${DOWNLOAD_TESTNET_BLOCKCHAIN} == true ]; then

        echo "" && echo "[INFO] downloading testnet blockchain ..."
        wget https://www.jelurida.com/NXT-nxt_test_db.zip -q --show-progress

        echo "" && echo "[INFO] unzipping testnet blockchain ..."
        unzip -qq NXT-nxt_test_db.zip

        echo "" && echo "[INFO] moving testnet blockchain to NXT testnet folder ..."
        mv nxt_test_db/ nxt-testnet/
    fi
fi
...
rm -rf nxt install-nxt.sh *.zip *.zip.asc *.txt
